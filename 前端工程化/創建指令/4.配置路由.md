...
function readMeta(filePath) {
  ...
  const metaPath = "../views" + filePath.replace(regex, "/meta.json")
...
async function printDirectoryContents(directory, parentPath = "/", arr = []) {
  const files = fs.readdirSync(directory)
  const hasLayout = files.includes("layout.tsx")
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      await printDirectoryContents(filePath, folderPath, arr)
    } else {
...
const createPathArray = async () => {
  ...
  let arr = []
  await printDirectoryContents(viewsDirectory, "/", arr)
  return arr.sort((a, b) => {
...
import ${component} from "${path}"`
    ...
    if (!isLazy) initOutput += isNotLazyImport
    if (meta?.lazy) delete meta.lazy
    const item = {
...
const createRouter = async () => {
  const arr = await createPathArray()

  const { output, targetPath } = createOutput(arr)
