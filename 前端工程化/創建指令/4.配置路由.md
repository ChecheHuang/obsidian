
```bash
npm i react-router-dom

mkdir -p src/router 
mkdir -p src/router/LazyLoad
mkdir -p src/views 
mkdir -p /src/views/one
mkdir -p /src/views/second
mkdir -p /src/views/one/onechild
mkdir -p /src/views/second/secondchild

echo "import { env } from '@/api/env'

const Home = () => {
  return (
    <button
      onClick={() => {
        env(import.meta.env)
          .then((res) => {
            console.log(res.data)
          })
          .catch((err) => {
            console.error(err)
          })
      }}
      className=\"shadow-md font-bold underline text-primary-dark\"
    >
      Hello world!
    </button>
  )
}

export default Home" > src/views/page.tsx

echo "import { Outlet } from 'react-router-dom'

const Layout = () => {
  return (
    <section className=\"flex h-screen\">
      <aside className=\"w-64 bg-gray-200\">aside</aside>
      <section className=\"flex-1 flex flex-col\">
        <header className=\"h-16 bg-blue-500\">header</header>
        <main className=\"p-4 max-h-screen overflow-y-auto\">
          <Outlet />
        </main>
      </section>
    </section>
  )
}

export default Layout" > src/views/layout.tsx

echo "const One = () => {
  return <div>One</div>
}

export default One" > src/views/one/page.tsx

echo "import { Outlet } from 'react-router-dom'

const Layout = () => {
  return (
    <div>
      OneLayout
      <Outlet />
    </div>
  )
}

export default Layout" > src/views/one/layout.tsx

echo "const OneChild = () => {
  return <div>OneChild</div>
}

export default OneChild" > src/views/one/onechild/page.tsx

echo "const Second = () => {
  return <div>Second</div>
}

export default Second" > src/views/second/page.tsx

echo "const SecondChild = () => {
  return <div>SecondChild</div>
}

export default SecondChild" > src/views/second/secondchild/page.tsx


echo '@keyframes loader-anim {
  0% {
    width: 0%;
  }

  100% {
    width: 100%;
  }
}

.animateLoader {
  animation: loader-anim 2s infinite;
}' > src/router/LazyLoad/loader.module.scss

echo "import React, { Suspense } from 'react'
import clsx from 'clsx'
import styles from './loader.module.scss'
export default function LazyLoad(
  ComponentPromise: Promise<{ default: React.ComponentType<object> }>
) {
  const LazyComponent = React.lazy(() => ComponentPromise)

  return (
    <Suspense fallback={<Loader />}>
      <LazyComponent />
    </Suspense>
  )
}

function Loader() {
  return (
    <div className=\"fixed top-0 left-0 w-full h-1  bg-gray-400\">
      <div
        className={clsx(
          'h-full bg-gradient-to-r from-green-500 to-blue-500',
          styles.animateLoader
        )}
      ></div>
    </div>
  )
}" > src/router/LazyLoad/LazyLoad.tsx


echo "import LazyLoad from './LazyLoad/LazyLoad'
const router = [
  {
    path: '/',
    element: LazyLoad(import('@/views/layout')),
    children: [
      {
        path: '',
        element: LazyLoad(import('@/views/page')),
      },
      {
        path: 'one',
        element: LazyLoad(import('@/views/one/layout')),
        children: [
          {
            path: '',
            element: LazyLoad(import('@/views/one/page')),
          },
          {
            path: 'onechild',
            element: LazyLoad(import('@/views/one/onechild/page')),
          },
        ],
      },
      {
        path: 'second',
        element: LazyLoad(import('@/views/second/page')),
      },
      {
        path: 'second/secondchild',
        element: LazyLoad(import('@/views/second/secondchild/page')),
      },
    ],
  },
  {
    path: '/*',
    label: '404',
    element: <div>404</div>,
  },
]

export default router" > src/router/router.tsx

echo "import { useRoutes } from 'react-router-dom'
import { BrowserRouter } from 'react-router-dom'

import router from '@/router/router'
const Routes = () => {
  const routes = useRoutes(router)
  return routes
}

function App() {
  return (
    <BrowserRouter>
      <Routes />
    </BrowserRouter>
  )
}

export default App" > src/App.tsx


```